...
myMovies.post("/", async (req, res, next) => {
  try {
    const username = await ensureUser(req.body.username);
    ...
    await q(
      `INSERT INTO user_movies (username, "Title", "Year", "Runtime", "Genre", "Director", "Favourite")
       VALUES ($1,$2,$3,$4,$5,$6,$7)
       ON CONFLICT (username, "Title") DO UPDATE SET
         "Year"=$3, "Runtime"=$4, "Genre"=$5, "Director"=$6, "Favourite"=$7, updated_at=now()`,
      [username, Title, Year, Runtime, Genre, Director, Favourite]
    );
    res.json({ ok: true });
  } catch (e) {
    next(e);
...
myMovies.delete("/", async (req, res, next) => {
  try {
    ...
    await q(`DELETE FROM user_movies WHERE username=$1 AND "Title"=$2`, [
      username,
      Title,
    ]);
    res.json({ ok: true });
